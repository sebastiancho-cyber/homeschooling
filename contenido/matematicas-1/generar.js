...
/* Nombre de la migración que se va a escribir. Se cambia en cada siembra:
   las migraciones son historia y no se reescriben. */
const MIGRACION = process.argv[2] ?? "0023_sin_fugas";
...
/* Tres versiones de cada pregunta.

   Una lección se repite —al reprobarla, o para subir estrellas— y con una sola
   versión el niño acaba recordando "la segunda de la derecha" en vez de sumar.
   Cada ranura de la lección tiene ahora 3 versiones y se sortea una.

   Las 10 ranuras por lección y las 39 evidencias del MEN no cambian: lo que
   crece es la profundidad de cada ranura. 10 temas × 10 ranuras × 3 = 300.

   Las versiones de cálculo salen de estas ayudas, que fabrican los distractores
   a partir del error típico y no de números al azar:
     suma(a,b)  → el distractor es restar en vez de sumar, y fallar por uno.
     resta(a,b) → el distractor es sumar en vez de restar, y fallar por uno.
     falta(a,c) → el distractor es sumar los dos números en vez de restarlos.
   La correcta va siempre de primera; el barajado lo hace el servidor. */

/* Los vecinos del resultado. NO son siempre +1 y −1: con ese molde, la
   correcta queda siempre en el medio de tres números seguidos y se acierta
   descartando el raro. El molde se elige con los propios operandos, así que
   varía entre ejercicios y no varía entre corridas. */
...
/* ---------- Ilustraciones ------------------------------------------------
   La ilustración se DERIVA del ejercicio, no se etiqueta a mano. Con 300
   ejercicios, etiquetarlos uno a uno garantiza que tarde o temprano el dibujo
   deje de corresponder al enunciado. Derivándola, cambiar el ejercicio cambia
   el dibujo solo.
...
/* ---------- Control de calidad ANTES de tocar la base ------------------- */
const errores = [];
...
for (const [dba, d] of Object.entries(DBAS)) {
  ...
  /* El reparto. Antes la única regla era "diez ranuras", y eso dejaba dos
     ...
     corta. */
  if (d.slots.length < 10) errores.push(`DBA ${dba}: ${d.slots.length} ranuras, mínimo 10`);
  const porEvidencia = {};
  d.slots.forEach((slot) => (porEvidencia[slot.e] = (porEvidencia[slot.e] ?? 0) + 1));
  d.evidencias.forEach((texto, k) => {
    const n = porEvidencia[k + 1] ?? 0;
    if (n < 2) errores.push(`DBA ${dba} ev${k + 1}: ${n} ranuras (mínimo 2) — «${texto.slice(0, 52)}…»`);
    if (n > 4) errores.push(`DBA ${dba} ev${k + 1}: ${n} ranuras (máximo 4) — acapara la lección`);
  });
  d.slots.forEach((slot, i) => {
    totalSlots++;
    if (slot.v.length !== 3) errores.push(`DBA ${dba} ranura ${i + 1}: ${slot.v.length} versiones, deben ser 3`);
    ...
    slot.v.forEach((v, j) => {
      ...
      v.o.forEach((o) => { if (o.length > 36) errores.push(`DBA ${dba}.${i + 1}.v${j + 1}: opción de ${o.length} caracteres — «${o}»`); });
      if (v.p.length > 70) errores.push(`DBA ${dba}.${i + 1}.v${j + 1}: enunciado de ${v.p.length} caracteres`);
    ...
    if (new Set(firmas).size !== 3) errores.push(`DBA ${dba} ranura ${i + 1}: versiones repetidas`);
    ...
    if (new Set(correctas).size === 1) {
      errores.push(`DBA ${dba} ranura ${i + 1}: las 3 versiones se contestan "${correctas[0]}"`);
    }
...
/* Una ranura queda MUDA si a la app no le llega ni una ayuda escrita ni una
   operación que sepa explicar. La ayuda se busca por el enunciado de la primera
   versión, no por la posición: repartir las ranuras las mueve de sitio, y una
   ayuda que se queda en la posición vieja no falla, MIENTE. */
...
let sql = `-- Tres versiones de cada pregunta.
...
begin
  select id into v_subject from subjects where slug = 'matematicas';
  delete from exercises where evidence_id in (
    select le.id from learning_evidences le join dbas d on d.id = le.dba_id
    where d.subject_id = v_subject and d.grade = 1);
  delete from learning_evidences where dba_id in (
    select id from dbas where subject_id = v_subject and grade = 1);
end $$;
...
for (const dbaNum of Object.keys(DBAS).map(Number).sort((a, b) => a - b)) {
  ...
  // Los identificadores salen del propio contenido, no de un sorteo. Así dos
  // corridas del generador dan el MISMO SQL y al revisar un cambio se ve solo
  // lo que cambió de verdad, en vez de trescientas líneas de uuid nuevos.
  const ids = evidencias.map((_, i) => uuidDe(`matematicas|1|dba${dbaNum}|ev${i + 1}`));
  sql += `-- ===== DBA ${dbaNum}: ${evidencias.length} evidencias · ${slots.length} ranuras · ${slots.length * 3} ejercicios =====\n`;
  evidencias.forEach((texto, i) => {
    sql += `insert into learning_evidences (id, dba_id, num, texto) select ${q(ids[i])}::uuid, d.id, ${i + 1}, ${q(texto)} from dbas d join subjects s on s.id = d.subject_id and s.slug = 'matematicas' where d.grade = 1 and d.num = ${dbaNum};\n`;
  });
...
begin
  select count(*) into n_ev from learning_evidences le join dbas d on d.id = le.dba_id
    join subjects s on s.id = d.subject_id and s.slug = 'matematicas' where d.grade = 1;
  select count(*) into n_ex from exercises e join learning_evidences le on le.id = e.evidence_id
    join dbas d on d.id = le.dba_id join subjects s on s.id = d.subject_id and s.slug = 'matematicas'
    where d.grade = 1;
  select count(*) into n_malas from exercises e join learning_evidences le on le.id = e.evidence_id
    join dbas d on d.id = le.dba_id join subjects s on s.id = d.subject_id and s.slug = 'matematicas'
    where d.grade = 1 and (jsonb_array_length(e.config->'options') <> 4
  ...
  select count(*) into n_ranuras from (
    ...
    where d.grade = 1 group by d.num, e.order_in_lesson having count(*) <> 3) t;
...
const out = __dirname + "/../../supabase/migrations/" + MIGRACION + ".sql";
fs.writeFileSync(out, sql, "utf-8");
fs.writeFileSync(__dirname + "/" + MIGRACION + ".body.json", JSON.stringify({ query: sql }), "utf-8");
